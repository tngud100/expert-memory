...
onMounted(async () => {
  ...
  const userData = await getUserDataByLoginId(loginId);
  id.value = userData.login_ID;
  name.value = userData.user_NAME;
  birthday.value.year = userData.birthday.split("-")[0];
  birthday.value.month = userData.birthday.split("-")[1];
  birthday.value.date = userData.birthday.split("-")[2];
  email.value.front = userData.email.split("@")[0];
  email.value.back = userData.email.split("@")[1];
  phoneNumber.value = userData.phone_NUM;
  address.value.zipcode = userData.zipcode;
  address.value.roadAddress = userData.road_ADDRESS;
  address.value.jibunAddress = userData.jibun_ADDRESS;
  address.value.detailAddress = userData.detail_ADDRESS;
});
</script>

<style src="@/assets/css/myinfo/SignForm.css" module>
</style>